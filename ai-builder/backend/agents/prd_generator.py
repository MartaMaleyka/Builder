...
class PRDGenerator:
    ...
    async def generate(self, context: ClarifyContext) -> PRDDocument:
        ...
        print(f"[PRDGenerator] tech_preferences: {context.tech_preferences!r}")
        messages = [
        ...
        try:
            ...
            print(f"[PRDGenerator] Structured output failed: {exc}. Using JSON fallback.")
    ...
    def _log_token_usage(raw_message) -> None:
        """Log token usage from Ollama response metadata."""
        if raw_message is None:
            return
        metadata = getattr(raw_message, "response_metadata", {}) or {}
        usage = metadata.get("token_usage") or metadata.get("usage") or {}
        if not usage:
            print("[PRDGenerator] Token usage not available in response metadata")
            return
        prompt = usage.get("prompt_tokens", usage.get("input_tokens", "?"))
        completion = usage.get("completion_tokens", usage.get("output_tokens", "?"))
        total = usage.get("total_tokens", "?")
        print(
            f"[PRDGenerator] Tokens — prompt: {prompt}, "
